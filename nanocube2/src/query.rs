    ...
    S: Monoid + PartialOrd + Copy,
{
    ...
    dim_range_list.push((Range::new_from_width(nc.dims[dim].width), nc.base_root));
    ...
    while dim < nc.dims.len() - 1 {
        ...
        while dim_range_list.len() > 0 {
            ...
            {
                next_dim_range_list.push((
                    Range::new_from_width(nc.dims[dim + 1].width),
                    current_node.next,
                ));
                continue;
            }
